...
drugs = {"타이레놀": {
    "side_effects": "쇽 증상(호흡곤란, 온몸이 붉어짐, 혈관부기, 두드러기 등), 천식발작, 혈소판감소, 과립구감소, 용혈성빈혈, 메트헤모글로빈혈증, 혈소판기능 저하(출혈시간 연장), 청색증, 구역, 구토, 식욕부진, 위장출혈, 소화성궤양, 천공(뚫림) 과민증상(얼굴부기, 땀이 남, 저혈압), 발진, 피부점막안증후군(스티븐스-존슨증후군), 독성표피괴사용해(리엘증후군), AST 상승, ALT 상승, 고정발진 등이 나타나는 경우 즉각 복용을 중지하고 의사 또는 약사와 상의하십시오."}}
...
DrugDetail_put_args.add_argument("frequency_per_day", type=str,
                                 help="How many times a day should you take the medicine", required=True)
DrugDetail_put_args.add_argument("treatment_period", type=str,
                                 help="How many days does this medication should be taken", required=True)

config = {'lang': 'en'}
def setLanguageTo(language):
    config['lang'] = str(language)
...
class PrescriptionInfo(Resource):
    ...
    def get(self, user_id, prescription_id):
        ...
        for dict in usersdb[user_id]["prescriptions"][prescription_id]:
            ...
            messages[drug_key] = "You should take {} of {}, {} for a period of {}".format(dosage, name, frequency,
                                                                                          period)
            drug_key += 1
        return messages
...
class AddPrescription(Resource):
    def put(self, user_id, prescription_id, drug_id):
        ...
        jsonfile = open('usersdb.json','r', encoding="utf-8")
        usersdb = json.load(jsonfile)

        if drug_id == 1:
            usersdb[str(user_id)]["prescriptions"][str(prescription_id)] = []
            usersdb[str(user_id)]["prescriptions"][str(prescription_id)].append(args)
        else:
            usersdb[str(user_id)]["prescriptions"][str(prescription_id)].append(args)
        with open('usersdb.json', 'w', encoding="utf-8") as outfile:
            json.dump(usersdb, outfile, ensure_ascii=False)
...
class DrugSideEffects(Resource):
    def get(self, drug_name):
        ...
        num_of_rows = 3
        response = requests.get(url, 'serviceKey={}&pageNo={}&numOfRows={}&itemName={}&type=json'.format(service_key, 1,
                                                                                                         num_of_rows,
                                                                                                         drug_name))
        ...
        if config['lang'] == 'en':
            side_effects = TranslateToEnglish(side_effects)
            drug_name = TranslateToEnglish(drug_name)
